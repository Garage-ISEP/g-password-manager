...
resource "aws_lambda_permission" "secrets-delete-permission" {
  ...
  source_arn = "arn:aws:execute-api:eu-west-3:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.api.id}/*/${aws_api_gateway_method.secrets-delete.http_method}${aws_api_gateway_resource.secrets.path}"
}
